...
TASK_CODE = 'sync_tracked_person_credits'


def create_sync_tracked_person_credits_task(apps, schema_editor):
    ScheduledTask = apps.get_model('config', 'ScheduledTask')
    ScheduledTask.objects.get_or_create(code=TASK_CODE)


def delete_sync_tracked_person_credits_task(apps, schema_editor):
    ScheduledTask = apps.get_model('config', 'ScheduledTask')
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('config', '0004_sync_kinopoisk_top250_task'),
    ]
    ...
    operations = [
        ...
        migrations.RunPython(
            create_sync_tracked_person_credits_task,
            delete_sync_tracked_person_credits_task,
        ),
    ]
