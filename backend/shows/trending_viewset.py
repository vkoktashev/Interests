...
def get_trending_shows_results(time_window='day'):
    key = f'tmdb_trending_shows_{time_window}_{LANGUAGE}'
    results = cache.get(key, None)
    if results is None:
        results = tmdb.Trending(media_type='tv', time_window=time_window).info(language=LANGUAGE)
...
class TrendingShowsViewSet(GenericViewSet, mixins.ListModelMixin):
    ...
    def list(self, request, *args, **kwargs):
        ...
        try:
            response = get_trending_shows_results(time_window)
